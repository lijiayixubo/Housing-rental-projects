<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb5">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:watermeter:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:watermeter:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:watermeter:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:watermeter:export']"
        >导出</el-button>
      </el-col>
      <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px" class="el-form-search">
                        ...
                        <el-form-item label="采集器id" prop="cid" class="el-form-search-item">
                            <el-input
    ...
    <el-table :height="tableHeight" v-loading="loading" :data="watermeterList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="采集器id" align="center" prop="cid" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="800px" append-to-body :close-on-click-modal="false" v-dialogDrag>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="采集器id" prop="cid">
          <el-input v-model="form.cid" placeholder="请输入采集器id" />
        </el-form-item>
...
<script>
import { listWatermeter, getWatermeter, delWatermeter, addWatermeter, updateWatermeter } from "@/api/system/watermeter";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listWatermeter(this.queryParams).then(response => {
        this.watermeterList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getWatermeter(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateWatermeter(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addWatermeter(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/watermeter/export', {
        ...this.queryParams
      }, `watermeter_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
