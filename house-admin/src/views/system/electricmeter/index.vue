<template>
  <div class="app-container">
    <el-row :gutter="10" class="mb5">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:electricmeter:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:electricmeter:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:electricmeter:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:electricmeter:export']"
        >导出</el-button>
      </el-col>
    ...
    <el-table :height="tableHeight" v-loading="loading" :data="electricmeterList" @selection-change="handleSelectionChange">
...
<script>
import { listElectricmeter, getElectricmeter, delElectricmeter, addElectricmeter, updateElectricmeter } from "@/api/system/electricmeter";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listElectricmeter(this.queryParams).then(response => {
        this.electricmeterList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getElectricmeter(id).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateElectricmeter(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addElectricmeter(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/electricmeter/export', {
        ...this.queryParams
      }, `electricmeter_${new Date().getTime()}.xlsx`)
    }
  }
};
</script>
