<template>
	<view class="page-container">
		<view class="top">
			<u-search class="banner" placeholder="你想住在哪儿" v-model="keyword" input-align="center"  :show-action="false" :clearabled="true"
			:disabled="true" height="80" style="width: 580rpx;" shape="square" @click="search"></u-search>
			<u-swiper :list="swiperList" height="350" @click="moreInfo"></u-swiper>
		</view>
		<view class="section">
			<view class="top-title">热门项目</view>
			<!-- 顶部导航栏 -->
					...
					<scroll-view class='scrollContainer' scroll-x>
						<view class='scrollitem' v-for="(item, index) in goodsArr" :key="index" @click="clickImage(item.id)">
							<image class="scrollimage" :src="item.image"></image>
							<view class="house-info">
								<view class="house-title">{{item.houseCode}}</view>
								<view class="housePrice">
									<view class="left">户型{{item.houseNum}}个</view>
									<view class="right">{{item.price}}元/{{item.payType[0]}}起</view>
								</view>
							</view>
		...
		<view class="section room">
			<view class="top-title">优选户型</view>
			<u-waterfall v-model="flowList" ref="uWaterfall">
			    <template v-slot:left="{leftList}">
			        <view class="demo-warter" v-for="(item, index) in leftList" :key="index">
			            <u-lazy-load style="width: 100%; height: 132px;" threshold="750" border-radius="12" :image="item.image" :index="index"
			    ...
			    <template v-slot:right="{rightList}">
			        <view class="demo-warter" v-for="(item, index) in rightList" :key="index">
			            <u-lazy-load style="width: 100%; height: 132px;" threshold="750" border-radius="10" :image="item.image" :index="index"
			                         @click="clickImage(item.id)"></u-lazy-load>
			            <view style="font-size: 14px;" class="item-title">{{item.villageName}} {{item.type == '整租' ? item.houseNum + item.houseHall + item.toiletNum : item.roomType}}</view>
			            <view class="item-price">¥{{item.price}}</view>
						 <view class="item-desc">{{item.type}} | {{item.type == '整租' ? item.houseArea : item.roomArea}}㎡ | {{item.decoration}} </view>
			        </view>
			    </template>
	...
	export default {
		data() {
			return {
				...
				swiperList: [
					{
						image: '/static/img/index/swiper/3.png',
					    title: '身无彩凤双飞翼，心有灵犀一点通'
					},
					{
						image: '/static/img/index/swiper/4.png',
					    title: '身无彩凤双飞翼，心有灵犀一点通'
					},
                ],
				noticeList: [],
		...
		methods: {
			...
			getBannerList(){
				let url = "/api/banner/list?pageNum=1&pageSize=20&orderByColumn=create_time&isAsc=desc";
				this.$u.get(url,{
					pageNum:1,
					pageSize:50,
					orderByColumn:'create_time',
