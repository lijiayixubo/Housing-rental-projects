<template>
    <view class="u-margin-left-20 u-margin-right-20">
        ...
        <view>
            <u-swiper :list="swiperlist" :height="350" mode="number" effect3d effect3d-previous-margin="0" :interval="5000" @click="clickImg"></u-swiper>
        </view>
		...
		<view class="navigation">
			...
			<view class="right" @click="bookItem">
				...
				this.$u.get(url, {
					...
					this.swiperlist = room.imageList.map(val=>{
						...
						if(!imgUrl.includes(config.staticUrl)){
							imgUrl = config.staticUrl+val.imagePath
						}else{
							imgUrl = val.imgUrl
						}
						return {
					...
					if(!room.agentAvatar || !room.agentAvatar.includes(config.staticUrl)){
