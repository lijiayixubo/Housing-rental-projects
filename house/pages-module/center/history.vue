<template>
	<view>
		<u-navbar :is-back="true" title="预约管理" :border-bottom="false">
		</u-navbar>
		<scroll-view scroll-y style="height: 100%;width: 100%;">
			<view class="page-box">
				...
				<view v-else class="order" v-for="(item, index) in list" :key="index"
				 @click="clickImage(item.id)">
					<view class="top">
						<view class="left">
							<view class="store">{{ item.id }}</view>
...
<script>
	...
	export default {
		...
		onLoad() {
			...
			let url = "api/houseApi/reservation/list";
			let data = {
				renterId: user?user.id:0,
			}
			this.$u.get(url,data).then(res => {
				res.rows.map(item => this.list.push(item));
