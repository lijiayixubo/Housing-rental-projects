<template>
	<view>
		<u-navbar :is-back="true" title="电费缴纳" :border-bottom="false"></u-navbar>
		<view class="wrap">
						...
						<view class="page-box">
							...
							<view v-else
							 class="order" v-for="(item, index) in swiper" :key="index">
								<view class="top">
									<view class="left">
										<view class="store">电费账单编号：{{ item.id }}</view>
								...
								<view class="item">
									...
									<view class="content">
										<view class="title u-line-2">账单金额：¥{{item.price}}</view>
	...
	export default {
		...
		methods: {
			...
			findAgreementList(idx) {
				let url = "/api/houseApi/getElectricList";
				let defaultData = {
					status:idx,
