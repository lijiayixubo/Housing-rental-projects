<template>
	<view>
		<u-navbar :is-back="true" title="租金缴纳" :border-bottom="false"></u-navbar>
		<view class="wrap">
						...
						<view class="page-box">
							...
							<view v-else
							 class="order" v-for="(item, index) in swiper" :key="index">
								<view class="top">
									<view class="left">
										<view class="store">租金账单编号：{{ item.id }}</view>
								...
								<view class="item">
									...
									<view class="content">
										<view class="title u-line-2">租金：{{item.price}}</view>
										<view class="title u-line-2">
											房间编号：{{item.houseId}}
										</view>
										<view class="price">租金周期：{{item.months}}</view>
										<view class="type">
											账单日期：{{item.time}}
										</view>
									</view>
	...
	export default {
		...
		methods: {
			...
			findAgreementList(idx) {
				let url = "/api/houseApi/getRentList";
				let defaultData = {
					status:idx,
				...
				this.$u.get(url, {...defaultData,...this.searchData}).then(result => {
					...
					for (let i = 0; i < this.agreementList.length; i++) {
						...
						item.months = item.month.split(',').map(m => {
							return (m-0+1)+'月 '
						}).join('');
